...
# Debian / Ubuntu x64
fpm -s dir -t deb -C package --name openitcockpit-agent --vendor "it-novum GmbH" --license "Apache License Version 2.0" --config-files etc/openitcockpit-agent --architecture native --maintainer "<daniel.ziegler@it-novum.com>" --description "openITCOCKPIT Monitoring Agent and remote plugin executor." --url "https://openitcockpit.io" --before-install packages/preinst.sh --after-install packages/postinst.sh --before-remove packages/prerm.sh --version "$version" --depends gawk
