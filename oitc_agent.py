...
logfile_handler = RotatingFileHandler(logfile, mode='a', maxBytes=10 * 1024 * 1024, backupCount=2, encoding=None,
                                      delay=0)
        ...
        try:
            ...
            if pid not in (1, 2):
                try:
                    ...
                    print_verbose_without_lock(
                        "'%s' Process is not allowing us to get the parent process id!" % (str(pid)), True)
                ...
                if config['default']['processstats-including-child-ids'] in (1, "1", "true", "True"):
                    try:
                        ...
                        print_verbose_without_lock(
                            "'%s' Process is not allowing us to get the child process ids!" % (str(pid)), True)
            ...
            try:
                ...
                print_verbose_without_lock(
                    "'%s' Process is not allowing us to get the username option!" % (name if name != "" else str(pid)),
                    True)
            ...
            try:
                ...
                print_verbose_without_lock(
                    "'%s' Process is not allowing us to get the cmdline option!" % (name if name != "" else str(pid)),
                    True)
            ...
            try:
                ...
                print_verbose_without_lock(
                    "'%s' Process is not allowing us to get the CPU usage!" % (name if name != "" else str(pid)), True)
                # agent_log.error("'%s' Process is not allowing us to get the CPU usage!" % (name if name != "" else str(pid)))
            ...
            try:
                ...
                print_verbose_without_lock("'%s' Process is not allowing us to get memory usage information!" % (
                    name if name != "" else str(pid)), True)
            ...
            try:
                ...
                print_verbose_without_lock("'%s' Process is not allowing us to get the percent of memory usage!" % (
                    name if name != "" else str(pid)), True)
            ...
            try:
                ...
                print_verbose_without_lock(
                    "'%s' Process is not allowing us to get the num_fds option!" % (name if name != "" else str(pid)),
                    True)
            ...
            try:
                ...
                print_verbose_without_lock("'%s' Process is not allowing us to get the open_files option!" % (
                    name if name != "" else str(pid)), True)
            ...
            try:
                ...
                for logType in logTypes:
                    try:
                        ...
                        print_verbose_without_lock(
                            "An error occured during windows eventlog check with log type %s!" % (logType), True)
                ...
                for customkey in jdata[key]:
                    ...
                    if customkey == 'default':
                        if 'max_worker_threads' in jdata[key][customkey]:
                            newcustomchecks[customkey]['max_worker_threads'] = str(
    ...
    def do_GET(self):
        try:
            if 'auth' in config['default']:
                ...
                elif self.headers.get('Authorization') == 'Basic ' + config['default']['auth'] or config['default'][
                    'auth'] == "":
            ...
            if 'auth' in config['default']:
                if str(config['default']['auth']).strip() and self.headers.get('Authorization') == None:
                    ...
                    self.wfile.write(json.dumps(
                        self._process_post_data(data=self.rfile.read(int(self.headers['Content-Length'])))).encode())
                elif str(config['default']['auth']).strip():
                    self.do_AUTHHEAD()
                    self.wfile.write(self.headers.get('Authorization').encode())
                    self.wfile.write('not authenticated'.encode())
...
def check_alfresco_stats():
    ...
    alfrescostats = []
    if jmx_import_successfull and 'alfrescostats' in config['default'] and config['default']['alfrescostats'] in (
    1, "1", "true", "True", True):
        if file_readable(config['default']['alfresco-javapath']):
            try:
                uri = ("%s:%s%s" % (config['default']['alfresco-jmxaddress'], config['default']['alfresco-jmxport'],
                                    config['default']['alfresco-jmxpath']))
                alfresco_jmxConnection = JMXConnection("service:jmx:rmi:///jndi/rmi://" + uri,
                                                       config['default']['alfresco-jmxuser'],
                                                       config['default']['alfresco-jmxpassword'],
                                                       config['default']['alfresco-javapath'])
        ...
        if check_interval_counter >= check_interval:
            ...
            try:
                if config['default']['dockerstats'] in (1, "1", "true", "True") and 'running' not in docker_stats_data:
                    ...
                    thread.start()
                    # thread.join()
                if jmx_import_successfull and 'alfrescostats' in config['default'] and config['default'][
                    'alfrescostats'] in (1, "1", "true", "True", True):
                ...
                if system == 'linux':
                    if config['default']['qemustats'] in (1, "1", "true", "True") and 'running' not in qemu_stats_data:
                        ...
                        thread.start()
                    if config['default']['systemdservices'] in (
                    1, "1", "true", "True") and 'running' not in systemd_services_data:
...
def run_customcheck_command(check):
    ...
    """
    print_verbose('Start custom check "%s" with timeout %s at %s' % (
    str(check['name']), str(check['timeout']), str(round(time.time()))), False)
    agent_log.info('Start custom check "%s" with timeout %s at %s' % (
    str(check['name']), str(check['timeout']), str(round(time.time()))))
    ...
    try:
        ...
        try:
            ...
            cached_customchecks_check_data[check['name']]['error'] = 'Command timeout after ' + str(
    ...
    while not thread_stop_requested:
        ...
        for check_name in customchecks:
            if check_name != 'DEFAULT' and check_name != 'default':
                if 'command' in customchecks[check_name] and customchecks[check_name]['command'] != '' and (
                        'enabled' not in customchecks[check_name] or customchecks[check_name]['enabled'] in (
                1, "1", "true", "True", True)):
                    ...
                    if (round(time.time()) - cached_customchecks_check_data[check_name][
                        'last_updated_timestamp']) >= interval and 'running' not in cached_customchecks_check_data[
                        check_name]:
                        check = {
                            'name': check_name,
                            'command': command,
        ...
        while not thread_stop_requested:
            ...
            if len(cached_check_data) > 0:
                try:
                    ...
                    response = requests.post(oitc['url'].strip() + '/agentconnector/updateCheckdata.json', data=data,
                                             headers=headers, verify=False)
                    agent_log.info(response)

                    if response.content.decode('utf-8').strip() != '':
                        responseData = json.loads(response.content.decode('utf-8'))
                        if autossl and 'new_ca' in responseData and 'ca_checksum' in responseData and responseData[
                            'new_ca'] in (1, "1", "true", "True", True) and file_readable(
                                config['default']['autossl-ca-file']):
                            ...
                            with open(config['default']['autossl-ca-file'], 'r') as f:
                                ...
                                if responseData[
                                    'new_ca'] == ca_checksum:  # validates, that new ca request comes from old ca server
                    ...
                    print_verbose('An error occured while trying to notify your configured openITCOCKPIT instance!',
                                  True)
...
def process_webserver(enableSSL=False):
    ...
    if enableSSL:
        ...
        protocol = 'https'
        httpd.socket = ssl.wrap_socket(httpd.socket, keyfile=config['default']['keyfile'],
                                       certfile=config['default']['certfile'], server_side=True)
    elif autossl and file_readable(config['default']['autossl-key-file']) and file_readable(
            config['default']['autossl-crt-file']) and file_readable(config['default']['autossl-ca-file']):
        agent_log.info('SSL with custom certificate enabled')
        protocol = 'https'
        httpd.socket = ssl.wrap_socket(httpd.socket, keyfile=config['default']['autossl-key-file'],
                                       certfile=config['default']['autossl-crt-file'], server_side=True,
                                       cert_reqs=ssl.CERT_REQUIRED, ca_certs=config['default']['autossl-ca-file'])

    print_verbose("Server started at %s://%s:%s with a check interval of %d seconds" % (
    protocol, config['default']['address'], str(config['default']['port']), int(config['default']['interval'])), False)
    agent_log.info("Server started at %s://%s:%s with a check interval of %d seconds" % (
    protocol, config['default']['address'], str(config['default']['port']), int(config['default']['interval'])))
...
def create_new_csr():
    ...
    try:
        ...
        ssl_paths = [config['default']['autossl-csr-file'], config['default']['autossl-key-file'],
                     config['default']['autossl-crt-file'], config['default']['autossl-ca-file']]
...
def pull_crt_from_server(renew=False):
    ...
    with certificate_check_lock:
        if config['oitc']['url'] and config['oitc']['url'] != "" and config['oitc']['apikey'] and config['oitc'][
            'apikey'] != "" and config['oitc']['hostuuid'] and config['oitc']['hostuuid'] != "":
            try:
                csr = create_new_csr()
                ...
                response = requests.post(config['oitc']['url'].strip() + '/agentconnector/certificate.json', data=data,
                                         headers=headers, verify=False)
                if response.content.decode('utf-8').strip() != '':
                    ...
                    if 'checksum_missing' in jdata:
                        ...
                        print_verbose('Add old certificate checksum to request or recreate Agent in openITCOCKPIT.',
                                      False)
                    ...
                    if 'unknown' in jdata:
                        print_verbose('Untrusted agent! Try again in 1 minute to get a certificate from the server.',
                                      False)
                        agent_log.warning(
...
def check_auto_certificate():
    ...
    if file_readable(config['default']['autossl-crt-file']):  # repeat condition because pull_crt_from_server could fail
        ...
        with open(config['default']['autossl-crt-file'], 'r') as f:
            ...
            print_verbose("SSL Certificate expires in %s on %s (DD-MM-YYYY)" % (
            str(datetime.date(int(exp_year), int(exp_month), int(exp_day)) - datetime.datetime.now().date()).split(',')[
                0], exp_date), False)
            agent_log.info("SSL Certificate expires in %s on %s (DD-MM-YYYY)" % (
            str(datetime.date(int(exp_year), int(exp_month), int(exp_day)) - datetime.datetime.now().date()).split(',')[
                0], exp_date))

            if datetime.date(int(exp_year), int(exp_month),
                             int(exp_day)) - datetime.datetime.now().date() <= datetime.timedelta(
                    days_until_cert_warning):
    ...
    if file_readable(config['default']['autossl-ca-file']):
        ...
        with open(config['default']['autossl-ca-file'], 'r') as f:
            ...
            if datetime.date(int(exp_year), int(exp_month),
                             int(exp_day)) - datetime.datetime.now().date() <= datetime.timedelta(
                    days_until_ca_warning):
                print_verbose("CA Certificate expires in %s on %s (DD-MM-YYYY)" % (
                str(datetime.date(int(exp_year), int(exp_month), int(exp_day)) - datetime.datetime.now().date()).split(
                    ',')[0], exp_date), False)
                agent_log.info("CA Certificate expires in %s on %s (DD-MM-YYYY)" % (
                str(datetime.date(int(exp_year), int(exp_month), int(exp_day)) - datetime.datetime.now().date()).split(
                    ',')[0], exp_date))
                requestNewCertificate = True
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "h:i:p:a:c:vs",
                                   ["interval=", "port=", "address=", "config=", "customchecks=", "certfile=",
                                    ...
                                    "autossl-key-file", "autossl-ca-file", "dockerstats", "qemustats", "no-cpustats",
                                    "no-sensorstats", "no-processstats", "processstats-including-child-ids",
                                    "no-netstats", "no-diskstats", "no-netio", "no-diskio", "no-winservices", "verbose",
                                    "stacktrace", "help"])
